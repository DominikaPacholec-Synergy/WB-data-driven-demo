...
const TaskDetail = ({
  ...
  const context = useRunStore(
    (state) => state.executions[task.execId]?.context,
  );
  ...
  return (
    <section className="card task-detail">
      ...
      {/* Rows, labels, formatting and order all come from `profile.run.facts`. TODO: why 'facts'?*/}
      <dl className="facts">
        {facts.map((fact) => (
          <div key={fact.key}>
            <dt>{fact.label}</dt>
      ...
      {task.reviewerNote ? (
        <p className="task-detail__note">{task.reviewerNote}</p>
      ) : null}
...
export const TasksView = ({
  ...
  const taskOrder = allTaskIds.filter(
    (id) => tasks[id].profileId === profileId,
  );
  const open =
    openId && tasks[openId]?.profileId === profileId ? tasks[openId] : null;
    ...
    <div className="view">
      <header className="view__head">
        ...
        <p className="view__sub">
          {taskOrder.filter((id) => tasks[id].status === "pending").length}{" "}
      ...
      {taskOrder.length === 0 ? (
        <p className="empty">
          ...
          No tasks yet. Start a run in the builder — the workflow will pause
          here.
        </p>
      ) : (
